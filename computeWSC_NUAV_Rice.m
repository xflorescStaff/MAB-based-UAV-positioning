function [WSC_Val, DeltaComp] = computeWSC_NUAV_Rice(A, E, UAVs, dAB, gammaA, gammaJ, channelParam )
    ...
    OmegaJE = abs(ne_LOS*dJE).^(-alpha_AG);                 % Only considering LOS component
    %gJE = 1./LJE;
    ...
    OmegaJB = abs(ne_LOS*dJB).^(-alpha_AG);                 % Only considering LOS component
    %gJB = 1./LJB;
    %intJB = gammaJ*sum(gJB,1);
    
    OmAB = gammaA*OmegaAB;
    OmvB = gammaJ*OmegaJB';
    KvB = KbVal*ones(1,nUAV);
    
    
    OmAE = gammaA*OmegaAE';
    OmvE = gammaJ*OmegaJE';
    KvE = KeVal*ones(1,nUAV);
    
    SOP_J = SOP_numint(Rs, OmAB, OmvB, KvB, OmAE, OmvE, KvE);
    ...
    SOP_NJ  = 1 - (exp( -(1./OmAB).*(2^Rs - 1) ))*( 1 ./ ( (2^Rs)*(OmAE./OmAB) + 1 ) );
    %SOP_NJ2  =transpose( 1 - (exp( -(OmegaAB2./aNJ).*(2^Rs - 1) ))*( 1 ./ ( (2^Rs)*(OmegaAB2./OmegaAE2).*(bNJ./aNJ) + 1 ) ));
